...
// AddCardForm component using Stripe Elements
const AddCardForm = ({ onSuccess, onCancel }: { onSuccess: () => void; onCancel: () => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (paymentMethod) {
        // Import billing API dynamically to avoid circular dependency
        const { billing } = await import('@/lib/api');
        await billing.addPaymentMethod(paymentMethod.id);
        onSuccess();
      }
    } catch (err: any) {
                  ...
                  <Elements stripe={stripePromise}>
                    <AddCardForm 
                      onSuccess={async () => {
                        ...
                        setBillingData((prev: any) => ({ ...prev, paymentMethods: methodsRes.paymentMethods }));
                      }}
